...
def create_vocab(data, num_steps, vocab_path):
    ...
    freq2_text = [word for word, freq in tokens_freq if freq > 1]
    ...
    # 将二维列表原文转为二维整数序列
    text_as_int = [
        [token_to_ix.get(ch, unk_idx) for ch in line] for line in tokenized_data
    ]
...
def prepare_data(sequence_length=8):
    ...
    class TranslateDataset(Dataset):
        ...
        def __getitem__(self, idx):
            return (
                torch.tensor(self.src_input_seqs[idx], dtype=torch.long),
