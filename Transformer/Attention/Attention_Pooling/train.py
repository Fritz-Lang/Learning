...
from utils import plot_kernel_reg, plot_truth
...
keys = X_tile[(1 - torch.eye(Config.n_train)).type(torch.bool)
              ].reshape((Config.n_train, -1))
# values的形状:('n_train'，'n_train'-1)
values = Y_tile[(1 - torch.eye(Config.n_train)).type(torch.bool)
                ].reshape((Config.n_train, -1))
